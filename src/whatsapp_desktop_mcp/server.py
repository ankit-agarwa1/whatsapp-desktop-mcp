...
# FORK FIX: this value must be resolvable BEFORE this module's body executes,
# because the `if not read_only_mode:` gate below runs AT IMPORT TIME. cli.py
# used to import this module and only THEN assign `server.read_only_mode`,
# which is too late — the gate had already been evaluated against the default,
# so `--no-read-only` never registered send_message and the send feature was
# dead. An env var is the only channel available before module execution.
# cli.py sets it before importing this module; the attribute is still assigned
# afterwards so `server.read_only_mode` stays the readable source of truth.
read_only_mode: bool = os.environ.get("WHATSAPP_DESKTOP_MCP_READ_ONLY", "1") != "0"
