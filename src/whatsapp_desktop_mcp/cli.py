...
def main(argv: list[str] | None = None) -> int:
    ...
    # FORK FIX: the env var must be set BEFORE the import below, because
    # importing `server` executes its module body, and that body contains the
    # `if not read_only_mode:` gate that registers send_message. Assigning the
    # attribute after the import (as this code used to do) is too late.
    os.environ["WHATSAPP_DESKTOP_MCP_READ_ONLY"] = "1" if args.read_only else "0"
