...
_INVARIANTS: list[tuple[str, str, int]] = [
    ...
    ("D-31 install script command", r"\./scripts/install\.sh", 1),
    ("D-31 send extra install flag", r"--with-send", 1),
    (
        "D-31 fixed venv path (TCC grants key on absolute path)",
        r"~/\.local/share/whatsapp-desktop-mcp/venv",
        1,
    ),
    ("D-31 MCP registration command", r"claude mcp add", 1),
    (
        "FORK-01 root cause of the macOS 15+ build failure is documented",
        r"CGWindowListCreateImageFromArray",
        1,
    ),
    ("FORK-02 wheels-vs-source-build explanation present", r"wheels", 1),
